...
# Older history archived in write-SiT5721_history.txt (untracked, local only)
new_pull_value         = 0.000000000  # default 0.000000000
#new_pull_value         = 1.40458797E-07  # 17d calculated end 2025-11-28, 19:04:42+00:00 with trend
#new_pull_value         = 1.31595357E-07  # 17d calculated end 2026-03-11, 19:16:22+00:00 with trend
new_pull_value         = 1.24157120E-07  # 15d calculated end 2026-03-28, 19:16:22+00:00 with trend

target_pull_value         = 0.000000000  # default 0.000000000
#target_pull_value         = 1.39454720E-07  # 17d calculated end 2025-11-28, 19:04:42+00:00 with trend
#target_pull_value         = 1.31595357E-07  # 17d calculated end 2026-03-11, 19:16:22+00:00 with trend
target_pull_value         = 1.23264846E-07  # 15d calculated end 2026-03-28, 19:16:22+00:00 with trend
...
#new_aging_compensation = -6.97342370E-16  # 17d calculated end 2025-11-28, 19:04:42+00:00 with trend
#new_aging_compensation = -6.97342370E-16  # 17d calculated end 2026-03-11, 19:16:22+00:00 with trend
new_aging_compensation = -6.97342370E-16  # 15d calculated end 2026-03-28, 19:16:22+00:00 with trend
...
class SiT5721:
    ...
    def read_SiT_config(self):    
        ...
        self.pull_value = float(''.join([str(item) for item in 
            struct.unpack('f', bytes(bus.read_i2c_block_data(address, 0x61, 4)))
            ]))
            
        ### 0x62, Pull Range, 4 bytes, Float, Fractional Offset, R/W, Default 10E-06
        self.pull_range = float(''.join([str(item) for item in 
            struct.unpack('f', bytes(bus.read_i2c_block_data(address, 0x62, 4)))
            ]))
        ...
        self.aging_compensation = float(''.join([str(item) for item in 
            struct.unpack('f', bytes(bus.read_i2c_block_data(address, 0x63, 4)))
            ]))
        ...
        self.max_freq_ramp_rate = float(''.join([str(item) for item in 
            struct.unpack('f', bytes(bus.read_i2c_block_data(address, 0x64, 4)))
            ]))
    ...
    def set_pull_value_from_target(self, target_pull_value):
        self.set_pull_value(self.calc_SiT_new_pull_value_from_target(target_pull_value))
    ...
    def set_pull_range(self, new_pull_range):
        bus.write_i2c_block_data(address, 0x62, list(struct.pack('f', new_pull_range)))
    ...
    def set_max_freq_ramp_rate(self, new_max_freq_ramp_rate):
        bus.write_i2c_block_data(address, 0x64, list(struct.pack('f', new_max_freq_ramp_rate)))
    ...
    def print_SiT_operation(self):
        ...
        print("Supply voltage          {:.8g} V".format(self.supply_voltage_float), end='\n')
        print("Resonator temperature {:=3.8g}°C".format(self.temperature_float), end='\n')
        print("Temperature error      {:=+3.8g}°C".format(self.temperature_err_float), end='\n')
        print("Heater power            {:.8g} W".format(self.heater_power_float), end='\n')
        print("Target power            {:.8g} W".format(self.heater_power_target_float), end='\n')
        print()
    ...
    def print_SiT_dynamic(self):
        ...
        print()
        print("Pull Value             {:=+.8g} ppm".format(self.pull_value / pow(10, -6)), end='\n')
        print("Pull Range              {:=.8g} ppm".format(self.pull_range / pow(10, -6)), end='\n')
        print("Aging compensation     {:=+.8g} part/s".format(self.aging_compensation), end='\n')
        #print("Aging compensation     {:=+.8g} ppb/s".format(self.aging_compensation / pow(10, -9)), end='\n')
        print("Max. Freq Ramp Rate     {:=.8g} ppm".format(self.max_freq_ramp_rate / pow(10, -6)), end='\n')
        print()
        print("Total offset written   {:=+.8g} ppm".format(self.total_offset_written / pow(10, -6)), end='\n')

    def print_SiT_derived(self):
        print("Target Pull Value      {:=+.8g} ppm".format(target_pull_value / pow(10, -6)), end='\n')
        print("Current Compensation   {:=+.8g} part/s".format(self.calc_SiT_current_compensation()), end='\n')
        print("New Pull Value         {:=+.8g} ppm".format(self.calc_SiT_new_pull_value_from_target(target_pull_value) / pow(10, -6)), end='\n')
